...
class Relay extends Server {
  ...
  async openUpstreamConnection (ds, clientAddr) {
    const options = {
      ...
      onMsaCode: (code) => {
        if (this.options.onMsaCode) {
          ...
          ds.disconnect(TextFormat.RED + `Hi ${ds.profile.name}! You are not registered with us yet, \nplease link your Microsoft account:\n\n` + code.message);
        }
      },
    ...
    client.on('close', (reason) => {
      ds.disconnect(TextFormat.RED + 'Sorry, there was an error with the connection to the server')
      this.upstreams.delete(clientAddr.hash)
    })
